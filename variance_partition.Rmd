...
#load reads:
#If tpms:
geneExp <- read.table("mynd.filtered.tpms.txt", header = TRUE, row.names = 1, check.names = F)

#Load covariates
info <- read.table("filtered_metadata.txt", header = TRUE, row.names = 1)
...
form <- ~ (1|status) + (1|gender) + (1|process_location) + (1|ethnicity) + (1|rna_batch) + (1|library_batch) + (1|lane) + (1|season) + age + rin_value +  pbmc_counts + monocyte_count + PCT_RIBOSOMAL_BASES + PCT_CODING_BASES + PCT_USABLE_BASES + PCT_INTERGENIC_BASES + PERCENT_DUPLICATION + MEDIAN_5PRIME_BIAS

#Variables to do the diagram:
form_1 <- ~ status + gender + process_location + ethnicity + rna_batch + library_batch + lane + season + age + rin_value + pbmc_counts + monocyte_count + PCT_RIBOSOMAL_BASES + PCT_CODING_BASES + PCT_USABLE_BASES + PCT_INTERGENIC_BASES + PERCENT_DUPLICATION + MEDIAN_5PRIME_BIAS  

#The variance partiation run:
varPart <- fitExtractVarPartModel( ge, form, info1 )
vp <- sortCols ( varPart )
plotVarPart ( vp )
status = varPart[order(varPart$status, decreasing=TRUE),]
head(status, 20)
plotPercentBars( status[1:20,] )
write.table(status, 'varpart.data.txt')
```
