...
#Distributions
```{r Distributions}
res = read.table("/sc/hydra/projects/pd-omics/mynd/analyses/analysis_9_19_19/differential_expression_11.04.2019/rsem.mynd.design_1.txt", header = T)
head(res)
pdf('pval.pdf')
...
x = read.table('/sc/hydra/projects/pd-omics/mynd/analyses/analysis_9_19_19/differential_expression_11.04.2019/rsem.mynd.design_1.txt', header = T, check.names  = F)
dim(x)
...
y = read.table('/sc/hydra/projects/pd-omics/mynd/analyses/analysis_9_19_19/mynd.tpms.txt', header = T, check.names = F)
#head(y)
...
total = merge(total, ann, by.x = "Row.names", by.y = 0)
dim(total)
rownames(total) = total$GeneSymbol
total$Row.names = NULL
total$logFC = NULL
total$GeneSymbol = NULL

#scale data
logTransformed.dat = as.matrix(total)
...
#Run heatmap2 for visualization - Regressing out covariates from tpm
pdf('hm.reg.pdf', width=30, height=16)
heatmap.2(
  z.matr,
  col = my_palette,
  Rowv = T,
...
## Correlation with Zhang Data
```{r Correlate with Zhang Data}
ann = read.table('/sc/hydra/projects/pd-omics/GRCH38.p12/ens.geneid.gencode.v30', header = T, check.names = F)
res_names = merge(ann, res, by.x = "gene_id", by.y = 0)

zhang = read.csv('/sc/hydra/projects/pd-omics/evan/MyND-Analysis/data/zhang_metaanlysis.csv', header =F)
head(zhang)

x1 = res_names[res_names$adj.P.Val < .05,]
table(zhang$V1 %in% x1$GeneSymbol)

cor_zhang = merge(x1, zhang, by.x = 'GeneSymbol', by.y = 'V1')
head(cor_zhang)
cor_zhang$V2 = gsub('DN', '-1', cor_zhang$V2)
cor_zhang$V2 = gsub('UP', '1', cor_zhang$V2)
cor_zhang$V2 = as.numeric(cor_zhang$V2)
cor.test(cor_zhang$logFC, cor_zhang$V3)
p = ggplot(cor_zhang, aes(logFC, V3, label = GeneSymbol))
p + geom_point() + geom_text_repel() + labs(title = "Zhang Correlation", subtitle = "r = 0.153, pvalue = 0.55", x = "mynd_log2FC", y = "zhang_beta") + 
  theme_bw() + geom_hline(yintercept = 0.0) + geom_vline(xintercept = 0.0)
```

## Correlation with Levadopa Equivalence
```{r ledd correlation}
x = read.csv('/sc/hydra/projects/pd-omics/evan/MyND-Analysis/data/ldopa_nyu_test.csv', header = T)
#x = x[x$ldopa > 0,]
x$samples = gsub('-.*', '', x$samples)
dim(x)

#Mitochondrial Gene List
mito = read.csv('/sc/hydra/projects/pd-omics/evan/MyND-Analysis/data/mito_genes.csv', header = F)

#Voom residuals
exp = read.table('/sc/hydra/projects/pd-omics/mynd/analyses/analysis_9_19_19/mynd.voom.residuals.txt', header = T, check.names = F)

colnames(exp) = gsub('-.*', '', colnames(exp))
colnames(exp) = gsub('_.*', '', colnames(exp))
rownames(x) = x$samples
rownames(x) %in% colnames(exp)
...
ldopa = data.frame(tot$ldopa)
rownames(ldopa) = rownames(tot)

res = data.frame(t((cor(ldopa$tot.ldopa, tot))), check.names = F)
colnames(res) = c("cor")
sig_de = res_names[res_names$adj.P.Val < .05,]
res$de = rownames(res) %in% sig_de$gene_id
res$index = c(1:nrow(res))
res$ldopa = NULL
p = ggplot(res, aes(index, cor, color = de))
p + geom_point() + scale_color_manual(values =c("black", "red")) + geom_hline(aes(yintercept= .006, linetype = "DGE Average r"), colour = 'red') +
    geom_hline(aes(yintercept= .011, linetype = "Overall average r"), colour= 'blue') +
    scale_linetype_manual(name = "Average Correlation",  values = c(2, 2), guide = guide_legend(override.aes = list(color = c("red", "blue")))) 

#mitochondrial
mito_filt = merge(res_names, mito, by.x = 'GeneSymbol', by.y = 'V2')
mitogene = mito_filt[c(2)]
rownames(mitogene) = mitogene$gene_id
mitocor = merge(mitogene, res, by = 0)

res1 = res_names
res1 = res1[res1$adj.P.Val < .05,]
mitocor = merge(res1, mitocor, by = 'gene_id')
sig_cor = merge(res, res1, by.x = 0, by.y = 'gene_id')
rownames(sig_cor) = sig_cor$Row.names
