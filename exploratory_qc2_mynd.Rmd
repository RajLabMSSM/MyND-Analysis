...
getResiduals <- function(exprLine) {
  ourCovRegression <- lm(exprLine ~ mynd_metadata$batch + mynd_metadata$rRNA.rate + mynd_metadata$gender + mynd_metadata$age + mynd_metadata$rin_value + mynd_metadata$process_location + mynd_metadata$ethnicity);
...
total$mismatch = ""
labelIndicesWeWant <- total$gender  == "M" & total$ENSG00000229807 > 5 | total$gender ==  "F" & total$ENSG00000183878 > 0
total$mismatch[labelIndicesWeWant] <- as.character(total$Row.names[labelIndicesWeWant])

p = ggplot(total, aes(x = ENSG00000229807, y = ENSG00000183878, color = gender, label = mismatch))
...
marker_genes = read.table('/sc/orga/projects/pd-omics/data/mynd/qc/cell.marker.expression', header = T, row.names = 1, check.names = F)
genenames = read.table('/sc/orga/projects/pd-omics/data/mynd/qc/ens_marker_ids', row.names = 1, header = F)
total = merge(marker_genes, genenames, by = 0, sort = F)
