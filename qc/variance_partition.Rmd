...
cpm = cpm(geneExp)
keep.exp = rowSums(cpm > 1) >= 0.3*ncol(geneExp)
ge = geneExp[keep.exp,]
#Variables to run Variance Partition with selected variables. Remember (1|..) for factors!
form <- ~(1|status) +  age + (1|gender) + (1|rna_batch) + mds1 + mds2 + mds3 + mds4 + PCT_RIBOSOMAL_BASES + rin_value + PCT_USABLE_BASES

#Variables to do the diagram for selected variables:
form_1 <- ~ status + age + gender + rna_batch + mds1 + mds2 + mds3 + mds4 + PCT_RIBOSOMAL_BASES + rin_value + PCT_USABLE_BASES 
